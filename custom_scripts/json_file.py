...
def create_example(json_file):
    ...
    for objects in list_of_objects:
        xmax.append(float(objects['maxcol'])/width)
        #track_id.append(member[0][2].text.encode('utf8'))
        xmin.append(float(objects['mincol'])/ width)
        ymin.append(float(objects['minrow'])/height)
        ymax.append(float(objects['maxrow'])/height)
...
def main(_):
    ...
    json_files = '/users/mscdsa2018/dsj1/PycharmProjects/action/data/tsinghuaDaimlerScripts/tar_folder/labelData/train/tsinghuaDaimlerDataset/*.json'
    ...
    for json_file in list:
        example = create_example(json_file)
